...
public:
	...
	void GetAsVectors_IFwd_JLeft_KUp(Vec3& out_forwardIBasis, Vec3& out_leftJBasis, Vec3& out_upwardKBasis) const;
	...
	bool  operator== (EulerAngles const& anglesToCompare) const;
	void  operator+= (EulerAngles const& anglesToAdd);
	void  operator-= (EulerAngles const& anglesToAdd);
	EulerAngles const operator*(float uniformScale) const;
	EulerAngles const operator-(EulerAngles const& anglesToSubtract) const;
	EulerAngles const RotateTowardsOrientation(EulerAngles const& targetOrientation, float maxRotation);
